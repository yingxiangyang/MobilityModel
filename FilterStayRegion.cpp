...
int main(int argc, char** argv){
		...
		if (per_count>personal_limit&&per_count<10000 && place_count>2){
			...
			if (home_valid_sign){
                for (int i=0;i<personal_count;i++){
                    ...
                    if (locs[i]==home){
                        fprintf(fout_id, "%d %d %d %d %f %f\n",counter1, times[i], locs[i], 1, lons[i], lats[i]);
                    }
                    else if (locs[i]==work){
                        fprintf(fout_id, "%d %d %d %d %f %f\n",counter1, times[i], locs[i], 2, lons[i], lats[i]);
                    }
                    else{
                        fprintf(fout_id, "%d %d %d %d %f %f\n",counter1, times[i], locs[i], 0, lons[i], lats[i]);
                    }
                }
			}
		}
	...
	cout << counter1 << " " << counter2 << " "<<counter3<<endl; //counter 1, with valid home, counter 2, with valid work
	return 0;
}
